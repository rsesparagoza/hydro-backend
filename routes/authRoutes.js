...
router.post('/register', async (req, res) => {
    const { firstName, lastName, username, email, password, confirmPassword, contactNumber } = req.body;
    const rainwater = 0;
    const deepwell = 0;
    const reservoir = 0;
    const phUp = 0;
    ...
    try {
        const existingUser = await User.findOne({ $or: [{ username }, { email }, { contactNumber }] });
        if (existingUser) {
            return res.status(422).send("Username/Email/Contact Number already exists");
        } else {
            const user = new User({
                userId: null,
                firstName,
            ...
            if (existingLevel) {
                ...
                if (existingReadings) {
                    ...
                    const existingControls = await Control.findOne({ $or: [{ username }] });
                    if (existingControls) {
                        return res.status(422).send("Username already exists");
                    } else {
...
router.post('/login',async (req,res)=>{
    ...
    if(!username || !password){
        return res.status(422).send({error :"You must provide username or password"})
    }
...
router.get('/user', async (req, res) => {
  ...
  router.post('/editProfile', async (req, res) => {
    ...
    try {
        const updatedUser = await User.findByIdAndUpdate(userId, { username, email, contactNumber }, { new: true });
...
router.post('/updatePassword', async (req, res) => {
    ...
    try {
        ...
        console.log("New profile data:", { oldPassword, password, confirmPassword });
...
router.get('/tankLevels', async (req, res) => {
    ...
    try {
        ...
        if (!level || level.length === 0) {
            return res.status(404).json({ error: 'Levels not found for the database' });
        }
...
router.post('/updateTankLevels', async (req, res) => {
    ...
    try {
        const updatedLevels = await Level.findByIdAndUpdate(levelId, { rainwater, reservoir, deepwell, phUp, phDown, nutrients }, { new: true });

        res.json({ message: "Tank Levels updated successfully", level: updatedLevels });
    } catch (error) {
        console.error("Error updating profile:", error);
        return res.status(500).send("Internal server error");
    }
...
router.get('/readings', async (req, res) => {
    ...
    try {
        ...
        if (!update || update.length === 0) {
            return res.status(404).json({ error: 'Updates not found for the database' });
        }
...
router.post('/updateReadings', async (req, res) => {
    ...
    try {
        const updatedReadings = await Update.findByIdAndUpdate(updatesId, { pHLevel, ppm, temp }, { new: true });

        res.json({ message: "Readings updated successfully", update: updatedReadings });
    } catch (error) {
  ...
  router.get('/controls', async (req, res) => {
    ...
    try {
        ...
        if (!control || control.length === 0) {
            return res.status(404).json({ error: 'Controls not found for the database' });
        }
...
router.post('/updateControls', async (req, res) => {
    ...
    try {
        const updatedControls = await Control.findByIdAndUpdate(controlsId, { rainwater, deepwell, reservoir, phUp, phDown, nutrients }, { new: true });

        res.json({ message: "Controls updated successfully", control: updatedControls });
    } catch (error) {
